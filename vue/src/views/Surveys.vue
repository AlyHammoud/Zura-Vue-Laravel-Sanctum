        ...
        <div
          ...
          <div class="flex justify-between items-center mt-3">
            ...
            <router-link :to="{name: 'SurveyView', params: {id: survey.id} }" 
              class="py-2 px-4 border border-transparent text-sm rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
            ...
            <!-- DeleteButton -->
            <button v-if="survey.id" type="button" @click="deleteSurvey(survey.id)"
...
function deleteSurvey(id){
  if(confirm(`Are you sure want to delete this survey? Operation can't be undone!!`)){
    // store.state.surveys.filter((surveryId) => surveryId !== id)
    store.commit('deteteSurvey', id);
  }
}
</script>
